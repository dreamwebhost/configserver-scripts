...
our ($class, $day, $hrs, $images, $min, $month, $myv, $script, $subdir, $used,
	$user, $versionfile, $x, $year, @localdomains, %FORM);
...
my %validdomains;
open (my $USERDOMAINS, "<", "/etc/userdomains") or die $!;
flock ($USERDOMAINS, LOCK_SH);
while (my $line = <$USERDOMAINS>) {
	chomp $line;
	my ($domain,$owner) = split(/:\s*/,$line,2);
	if (defined $domain and defined $owner and $domain ne "" and $owner ne "") {
		$validdomains{$domain} = $owner;
	}
}
close ($USERDOMAINS);

if ($FORM{domain} ne "" and $FORM{domain} =~ /[^\w\-\.]/) {
	print "Invalid domain name [".&html_escape($FORM{domain})."]";
}
elsif ($FORM{domain} ne "" and !exists $validdomains{$FORM{domain}}) {
	print "Invalid or non-local domain [".&html_escape($FORM{domain})."]";
}
elsif ($FORM{account} ne "" and (($FORM{account} =~ /[^a-zA-Z0-9\-\_\.\@\+]/) or ($FORM{account} eq ".") or ($FORM{account} eq ".."))) {
	print "Invalid account name [".&html_escape($FORM{account})."]";
}
	...
	foreach my $aliases (@forwarders) {
		...
		print "<tr><td>".&html_escape($alias)."</td><td>".&html_escape($recipient)."</td></tr>\n";
		$total++;
	}
	...
	if (defined $FORM{file} and $FORM{file} ne "") {
		my %userdomains;
		open (my $IN,"<","/etc/userdomains");
		flock ($IN, LOCK_SH);
		my @localusers = <$IN>;
		...
		my $owner = $userdomains{$FORM{domain}};
		my $homedir = ( getpwnam($owner) )[7];
		my $safefile = &mail_path($FORM{file}, "$homedir/mail");
		if ($homedir eq "" or !defined $safefile or !-f $safefile) {
			print "Invalid file [".&html_escape($FORM{file})."]";
		} else {
			...
			unless ($FORM{f}) {
				...
				unless (eof ($IN)) {
					...
					print "</pre>\n";
					print "...[truncated to 500 lines] <b><a href=\"$script?action=viewmail&amp;f=1&amp;domain=$FORM{domain}&amp;file=".&uri_escape($FORM{file})."\">View full ($size KB) email</a></b>\n";
				} else {
					print "</pre>";
				}
			} else {
		...
		print "File [".&html_escape($FORM{file})."] not found";
	}
}
	...
	if ($FORM{top}) {
		$dir = "$homedir/mail";
		$title = "$owner (cPanel user)";
	} else {
		$dir = "$homedir/mail/$FORM{domain}/$FORM{account}";
		$title = "$FORM{account}\@$FORM{domain}";
	}
	$dir = &mail_path($dir, "$homedir/mail");
	if (!defined $dir or !-d $dir) {print "Invalid mailbox"; exit}
	local $) = local $(; ## no critic
	local $> = local $<; ## no critic
	&drop($owner);
	...
	if ($topdir eq "/") {
		...
		print "		}\n";
		print "}\n</script>\n";
		print "<form action='$script' method='post' name='listmail'><input type='hidden' name='domain' value='$FORM{domain}'><input type='hidden' name='account' value='$FORM{account}'><input type='hidden' name='action' value='bulkdelete'><input type='hidden' name='topdir' value='".&html_escape($topdir)."'><input type='hidden' name='top' value='$FORM{top}'>\n";
		print "<table class='table table-bordered table-striped'>\n";
		print "<thead><tr><th colspan='6'>$title</th></tr></thead>";
		my $selecteddir_uri = &uri_escape("$dir$showdir");
		print "<tr><td colspan='6'><code>".&html_escape($showdir)."</code> <a class='btn btn-primary glyphicon glyphicon-level-up' href=\"$script?action=view&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;top=$FORM{top}\" data-tooltip='tooltip' title='Go Up'></a> <a class='btn btn-warning glyphicon glyphicon-trash' href=\"$script?action=emptydir&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;file=$selecteddir_uri\" data-tooltip='tooltip' title='Empty Directory' target='_blank' onClick='return confirmSubmit()'></a>";
		if ($mailscanner) {print " <a class='btn btn-success glyphicon glyphicon-flag' href=\"$script?action=salearn&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;file=$selecteddir_uri\" data-tooltip='tooltip' title='Run SA Learn for spam' target='_blank'></a>"}
		print "</td></tr>\n";
		...
		foreach my $subdir ("/cur","/new","/tmp") {
			...
			foreach my $file (sort @files) {
				if ((-f "$dir$line$subdir/$file") and ($file =~ /^(\d+)\./)) {
					...
					my $mailpath = "$dir$line$subdir/$file";
					my $mailpath_html = &html_escape($mailpath);
					my $mailpath_uri = &uri_escape($mailpath);
					print "<td><input type='checkbox' name='cmmdel_$total' value='$mailpath_html'></td>";
					print "<td><a class='btn btn-default modalButton' data-toggle='modal' data-src='$script?action=viewmail&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;file=$mailpath_uri' data-height='500px' data-width='100%' data-target='#myModal' href='#'>$subject</a></td><td>$date</td><td>$size KB</td>\n";
					print "<td><a class='btn btn-danger glyphicon glyphicon-remove modalButton' data-toggle='modal' data-src='$script?action=deletemail&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;file=$mailpath_uri' data-height='500px' data-width='100%' data-target='#myModal' href='#' data-tooltip='tooltip' title='Delete Email' onclick='\$(\"#tr$total\").hide()'></a></td></tr>\n";
				}
			}
			if (($topdir eq "/") and ($dirtot > 0)) {
				...
				print "<tr><td>$emptydir</td><td><code>".&html_escape($showdir)."</code></td><td><a class='btn btn-default' href=\"$script?action=view&amp;domain=$FORM{domain}&amp;account=$FORM{account}&amp;topdir=".&uri_escape($line.$subdir)."&amp;top=$FORM{top}\">View <span class='label label-pill label-info'>$dirtot</span> emails</a></td><td colspan='3'>$used MB</td></tr>\n";
			}
		}
	}
	if ($topdir ne "/") {
	...
	if (defined $FORM{file} and $FORM{file} ne "") {
		...
		if ($homedir eq "" or !defined $safefile or !-f $safefile) {
			...
			unlink $safefile;
			my ($file, $filedir) = fileparse($safefile);
	...
	if (defined $FORM{file} and $FORM{file} ne "") {
		...
		my $owner = $userdomains{$FORM{domain}};
		my $homedir = ( getpwnam($owner) )[7];
		my $safedir = &mail_path($FORM{file}, "$homedir/mail");
		if ($homedir eq "" or !defined $safedir or !-d $safedir) {
			print "Invalid directory [".&html_escape($FORM{file})."]";
		} else {
			local $) = local $(; ## no critic
			local $> = local $<; ## no critic
			&drop($owner);
			...
			my $total = 0;
			print "<table class='table table-bordered table-striped'>\n";
			print "<thead><tr><th>Empty Directory [".&html_escape($FORM{file})."]</th></tr></thead>";
			print "<tr><td>";
		...
		print "Directory [".&html_escape($FORM{file})."] not found";
	}
}
	...
	my $owner = $userdomains{$FORM{domain}};
	my $homedir = ( getpwnam($owner) )[7];
	if ($homedir eq "") {print "Invalid mailbox"; exit}
	local $) = local $(; ## no critic
	local $> = local $<; ## no critic
	&drop($owner);
	print "<table class='table table-bordered table-striped'>\n";
	...
	for ($x = 1; $x <= $FORM{total} ;$x++) {
		...
		my $safefile = &mail_path($delfile, "$homedir/mail");
		if (defined $safefile and -f $safefile) {
			my ($file, $filedir) = fileparse($safefile);
			if ($homedir eq "") {
				print "Invalid file [".&html_escape($delfile)."]";
			} else {
				unlink ($safefile);
				$total++;
				$anyfile = $safefile;
	...
	if (-d $filedir) {
		my $safedir = &mail_path($filedir, "$homedir/mail");
		if ($homedir eq "" or !defined $safedir) {
			print "Invalid directory [".&html_escape($filedir)."]";
		} else {
			if (-e "$filedir/../maildirsize") {unlink "$filedir/../maildirsize"}
		}
	}
	...
	eval {
		...
		foreach my $line (@data) {
			...
			if ($line =~ /^[\r\n]/) {next}
			if ($line =~ /^<br \/>/) {next}
			print "<pre>".&html_escape($line)."</pre>\n";
			$cnt++;
		}
	...
	if ($FORM{password} eq "") {
		...
		eval {
			...
			foreach my $line (@data) {
				if ($line =~ /^stdin: is not a tty/) {next}
				print "<div>".&html_escape($line)."</div>\n";
				$cnt++;
			}
...
#		unless (-e "$homedir/etc/$domain/quota") {next}
		...
		foreach my $key (@accounts) {
			...
			if ($FORM{action} eq "Manage Mail Accounts") {
				...
				$tot_mails+=$files;
				$tot_space+=$tused;
				print "<tr class='$uclass'><form action='$script' method='post'><td>$key_html$topdomain</td><td>$files</td><td>$used</td><td>";
				if ($first == -1) {
					print "$quota</td><td>";
					if ($files > 0 and !$mdbox) {
						print " <a class='btn btn-primary glyphicon glyphicon-folder-open' href='$script?action=view&amp;account=$key_uri&amp;domain=$domain&amp;top=1' data-tooltip='tooltip' title='View MailBox Contents'></a>";
						print " <a class='btn btn-warning glyphicon glyphicon-trash' href='$script?action=empty&amp;account=$key_uri&amp;domain=$domain&amp;top=1' data-tooltip='tooltip' title='Empty Mailbox'></a>";
					}
				} else {
					print "<input type='text' size='10' name='quota' value='$quota' style='text-align: right; padding-right: 2px'><input type='hidden' name='domain' value='$domain'><input type='hidden' name='account' value='$key_html'><input type='hidden' name='action' value='changequota'></td><td><button type='submit' class='btn btn-info glyphicon glyphicon-hdd' data-tooltip='tooltip' title='Change Quota'></button>";
					print " <a class='btn btn-success glyphicon glyphicon-lock' href='$script?action=password&amp;domain=$domain&amp;account=$key_uri' data-tooltip='tooltip' title='Change Mailbox Password'></a>";
					if ($files > 0 and !$mdbox) {
						print " <a class='btn btn-primary glyphicon glyphicon-folder-open' href='$script?action=view&amp;account=$key_uri&amp;domain=$domain' data-tooltip='tooltip' title='View MailBox Contents'></a>";
						print " <a class='btn btn-warning glyphicon glyphicon-trash' href='$script?action=empty&amp;account=$key_uri&amp;domain=$domain' data-tooltip='tooltip' title='Empty Mailbox'></a>";
					} else {print "";}
					print " <a class='btn btn-danger glyphicon glyphicon-remove' href='$script?action=delete&amp;domain=$domain&amp;account=$key_uri' data-tooltip='tooltip' title='Delete Mailbox'></a>";
				}
				print "</td></form></tr>\n";
			} else {
				print "<tr class='$uclass'><td>$key_html$topdomain</td><td>$files</td><td>$used</td><td>$quota</td><td>\n";
				print " <a class='btn btn-primary glyphicon glyphicon-cog' href='$script?action=Manage\%20Mail\%20Accounts&amp;account=$key_uri&amp;domain=$domain&amp;space=$FORM{space}&amp;percent=$FORM{percent}' data-tooltip='tooltip' title='Manage Mail Accounts'></td></tr>\n";
			}
...
sub drop {
	...
	if (!defined $uid or !defined $gid or !defined $home or $home eq "") {
		print "Invalid cPanel user\n";
		exit;
	}
	if ($> == 0) {
		chdir($home) or do {print "Failed to access cPanel user home\n"; exit};
		$( = $gid; ## no critic
		$) = "$gid $gid"; ## no critic
		$> = $< = $uid; ## no critic
...
sub mail_path {
	my ($path, $mailroot) = @_;
	return unless defined $path and defined $mailroot;
	return if $path eq "" or $mailroot eq "";
	my $realroot = realpath($mailroot);
	my $realpath = realpath($path);
	return unless defined $realroot and defined $realpath;
	return unless $realpath eq $realroot or index($realpath, "$realroot/") == 0;
	return $realpath;
}
###############################################################################
sub uri_escape {
